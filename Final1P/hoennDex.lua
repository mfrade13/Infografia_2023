...
local function requestData(e)
    if e.isError then
        ...
        sprite_url = data.sprites.versions["generation-iii"].emerald.front_default
...
local function requestDesc(e)
    if e.isError then
        ...
        for i =1, #des_list do
            if (des_list[i].language.name == "en") and (des_list[i].version.name=="emerald") then
                --Show this one 
...
local function textInputListener(event)
    ...
    elseif event.phase == "submitted" then
        ...
        native.setKeyboardFocus(nil) -- Oculta el teclado
        if (tonumber(textField.text) >= 252) and tonumber(textField.text) <= 386 then
            --Is a valid pkmn id
...
function scene:create( event )
    ...
    fondo = display.newImageRect(sceneGroup, assetsPath.."Hoenn/hoenn_Bg.jpg",CW, CH)
    fondo.x, fondo.y = 0, 0
    fondo.anchorX, fondo.anchorY = 0 , 0

    textField = native.newTextField((CW/2.5),(CH/25),200,40)
    ...
    next_btn = widget.newButton({
        ...
        width = 150,
        height = 50,
        defaultFile = assetsPath.."Hoenn/hoenn_btn.png",
        label = "NEXT",
    ...
    back_btn = widget.newButton({
        ...
        width = 150,
        height = 50,
        defaultFile = assetsPath.."Hoenn/hoenn_btn2.png",
        label = "BACK",
        labelColor = { default = { 0, 0, 0}},
        font = font,
        fontSize = 60,
